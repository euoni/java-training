...
class Option {
	...
	private final Preferences prefs = Preferences.userRoot().node("/euoni/digitalclock2");
	...
	public void createFromString(String font, String size, String fg, String bg) {
		this.font = new Font(font, Font.PLAIN, Integer.parseInt(size));
		this.foreground = ColorID.valueOf(fg).getColor();
		this.background = ColorID.valueOf(bg).getColor();
	}

	public void load() {
		createFromString(prefs.get("font", font.getFamily()), prefs.get("size", String.valueOf(font.getSize())),
				prefs.get("foreground", ColorID.fromColor(foreground).toString()),
				prefs.get("background", ColorID.fromColor(background).toString()));
	...
	public boolean save() {
		...
		prefs.put("foreground", ColorID.fromColor(foreground).toString());
		prefs.put("background", ColorID.fromColor(background).toString());
